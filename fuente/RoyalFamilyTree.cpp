...
bool CustomString::startsWith(const char* prefix) const {
...
bool CustomString::toBool() const {
...
void RoyalFamilyTree::_tokenizeLine(const CustomString& line, CustomString data[], int max_tokens) {
    int token_index = 0;
...
void RoyalFamilyTree::_loadFromCsv(const char* filepath) {
    ...
    // Conexión del Árbol (Punteros Parent/Child)
    Node* current = allNodesHead;
    while (current != nullptr) {
...
void RoyalFamilyTree::loadData(const char* filepath) { _loadFromCsv(filepath); }
